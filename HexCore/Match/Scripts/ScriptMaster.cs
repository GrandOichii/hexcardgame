...
namespace HexCore.GameMatch.Scripts;
...
public class ScriptMaster {
    ...
    public LuaTable SummonCard(string fromMID, string ownerID, string cID) {
        ...
        var cardSource = _match.GetCard(fromMID);
        var card = _match.CardMaster.Get(cID).GetAwaiter().GetResult();

        var newCard = new MatchCard(_match, card, player)
        {
            GoesToDiscard = false
        };
    ...
    public LuaTable? PickTile(string pID, string mID, LuaTable choices) {
        ...
        // TODO i dont know what will happen if i change this to async
        var result = player.Controller.PickTile(c, player, _match).GetAwaiter().GetResult();
